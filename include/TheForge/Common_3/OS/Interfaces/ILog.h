...
FailBehavior _FailedAssert(const char* file, int line, const char* condition);
FailBehavior _FailedAssert(const char* file, int line, const char* condition, const char* fmt, ...);
...
#elif defined(NX64)
#define  ASSERT(b, ...) \
	do if(!(b)) NN_ABORT_UNLESS(b, ##__VA_ARGS__); while(0)
#else
...
#define ASSERT(b, ...) \
	...
	{ \
		if( !(b) ) \
		{ \
			if( _FailedAssert(__FILE__, __LINE__, #b, ##__VA_ARGS__) == FailBehavior::Halt ) \
				halt(); \
		} \
	} while(0)
#endif
